...
class TudaqianFluxControlNetModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(
        ...
        # <<< START NEW CODE >>>
        # Learnable scaling factors for the two control conditions
        # Initialize alpha for the primary condition (controlnet_cond)
        self.control_alpha_scale = nn.Parameter(torch.tensor(1.0))
    ...
    def from_transformer(
        ...
        if load_weights_from_transformer:
            ...
            # <<< START NEW CODE >>>
            # Initialize the alternative embedder with zeros
    ...
    def forward(
        ...
        # This is the path assuming no hint block (conditions are VAE encoded)

        # --- Process and scale primary condition ---
        ...
        # --- Process and scale alternative condition (if provided) ---
        if controlnet_cond_alt is not None and hasattr(self, "controlnet_alt_x_embedder"):
