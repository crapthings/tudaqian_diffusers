...
from diffusers.models.controlnets.tudaqian_controlnet_flux import TudaqianFluxControlNetModel
...
def log_validation(
    ...
    if not is_final_validation:
        ...
        flux_controlnet = TudaqianFluxControlNetModel.from_pretrained(
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="conditioning_image_alt", # Default to the name in your dataloader
...
def get_train_dataset(args, accelerator):
    ...
    if args.conditioning_image_alt_column is None:
        ...
        try:
            conditioning_image_alt_column_index = column_names.index(conditioning_image_column) + 1
            conditioning_image_alt_column = column_names[conditioning_image_alt_column_index]
            logger.info(f"conditioning image alt column defaulting to {conditioning_image_alt_column}")
        except (ValueError, IndexError):
             raise ValueError(
                 "Could not infer `conditioning_image_alt_column`. Please specify it using `--conditioning_image_alt_column`. "
...
def prepare_train_dataset(dataset, accelerator):
    ...
    # <<< START NEW CODE >>>
    # Assuming the alt conditioning image needs the same transforms as the primary one
    conditioning_image_alt_transforms = transforms.Compose(
        [
            transforms.Resize(args.resolution, interpolation=transforms.InterpolationMode.BILINEAR),
            transforms.CenterCrop(args.resolution),
            transforms.ToTensor(),
    ...
    def preprocess_train(examples):
        ...
        # <<< START NEW CODE >>>
        conditioning_images_alt = [
            (image.convert("RGB") if not isinstance(image, str) else Image.open(image).convert("RGB"))
            for image in examples[args.conditioning_image_alt_column] # Use the new column name
        ]
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        flux_controlnet = TudaqianFluxControlNetModel.from_pretrained(args.controlnet_model_name_or_path)
    else:
        logger.info("Initializing controlnet weights from transformer")
        # we can define the num_layers, num_single_layers,
        flux_controlnet = TudaqianFluxControlNetModel.from_transformer(
            flux_transformer,
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                load_model = TudaqianFluxControlNetModel.from_pretrained(input_dir, subfolder="flux_controlnet")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_controlnet):
                ...
                control_alt_values = batch["conditioning_pixel_values_alt"].to(dtype=weight_dtype)
                control_alt_latents = vae.encode(control_alt_values).latent_dist.sample()
                control_alt_latents = (control_alt_latents - vae.config.shift_factor) * vae.config.scaling_factor
                # We now have 'control_image_alt' in latent space, ready to be used.
                control_image_alt = FluxControlNetPipeline._pack_latents(
                    control_alt_latents,
                    control_alt_values.shape[0],
                    control_alt_latents.shape[1],
                    control_alt_latents.shape[2],
                    control_alt_latents.shape[3],
                )
